...
/**
 * Lookup table for switching between plans.
 * First value of array if the change is possible.
 * Second value of array is the button label.
 * @type {Object}
 */
...
/**
 * Check if the user can upgrade/downgrade to the given plan ID
...
export function canChangeToPlan (activePlanId, targetPlanId) {
  ...
  // For cases where pricing table is disabled
  if (activePlan.label === ADMIN || activePlan.type === ENTERPRISE) {
    const [can, description] = changeLookupTable.disabled[targetSelector]
    return { can, description }
  }

  // For default cases
  if ([STARTER, TRIAL].includes(activePlan.label)) {
    const [can, description] = changeLookupTable.default[targetSelector]
    return { can, description }
  }
